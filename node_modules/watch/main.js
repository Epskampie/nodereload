...
exports.watchTree = function ( root, options, callback ) {
  ...
  walk(root, options, function (err, files) {
    ...
    var fileWatcher = function (f) {
      fs.watch(f, options, function (event, filename) {
        // console.log("Got watch event", event, filename);
        if (filename) {
          // Prevent duplicate events
          if (filename != lastFilename || (new Date().getTime() - lastTime) > 100) {
            // console.log('Sending event', event, filename);
